 ...
 */
static inline void _set_directedness(CSRGraph* graph, FILE* file) {
 ...
 * @param file The file to parse.
 */
static inline void _set_num_vertices_edges(CSRGraph* graph, FILE* file) {
    assert(graph != NULL && file != NULL);
 ...
 * @return int*
 */
static inline int* _get_directed_degrees(CSRGraph* graph) {
 ...
 */
static inline int* _get_undirected_degrees(CSRGraph* graph) {
    int* degrees = calloc(graph->num_vertices, sizeof(int));
 ...
 */
static inline int _get_relative_idx_col(CSRGraph* graph, int idx_row, int idx_col) {
    int* ptr_rows = graph->adjacency_matrix->ptr_rows;
    int* idx_cols = graph->adjacency_matrix->idx_cols;
 ...
 * @return CSRGraph* The newly created graph.
 */
 ...
 * @return CSRGraph* The newly created deep copy of the graph.
 */
CSRGraph* csr_graph_copy(CSRGraph* graph) {
    assert(graph != NULL);
    assert(graph->adjacency_matrix->is_set);

    CSRGraph* copy = csr_graph_new(graph->num_vertices, graph->num_edges, graph->is_directed);
 ...
 * then frees the graph and sets the pointer CSRGraph** graph to NULL.
 *
 ...
 * @return CSRGraph* The newly created directed graph.
 */
CSRGraph* csr_graph_make_directed(CSRGraph* graph, int (*f)(int, int, int*), int* meta_data) {
    ...
    CSRGraph* directed_graph = csr_graph_copy(graph);
    ...
    for (int i = 0; i < graph->num_edges; i++) {
        int vertex_u = idx_decompressed_und_rows[i];
        int vertex_v = idx_dir_cols[i];

        int target_vertex = f(vertex_u, vertex_v, meta_data);
        int source_vertex = target_vertex == vertex_u ? vertex_v : vertex_u;

        idx_decompressed_und_rows[i] = source_vertex;
        idx_dir_cols[i] = target_vertex;
    }
...
CSRGraph* csr_graph_reduce(CSRGraph* graph, bool* removed_vertices) {
    ...
    CSRGraph* reduced_graph = malloc(sizeof(CSRGraph));
 ...
 */
int csr_graph_get_edge(CSRGraph* graph, int idx_row, int idx_col) {
    assert(graph != NULL);
    assert(graph->adjacency_matrix != NULL);
    assert(graph->adjacency_matrix->is_set);
